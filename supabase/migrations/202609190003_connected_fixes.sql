-- Bring the connected schema in line with the date-only CRM model and enable recorded realtime subscriptions.
alter table public.prospects add column if not exists contract_end date;
alter table public.prospects alter column next_meeting type date using next_meeting::date;

drop policy if exists audit_select_active_member on public.audit_log;
create policy audit_select_admin on public.audit_log
for select to authenticated using (
  public.is_active_member(organization_id) and public.current_app_role() = 'admin'
);

do $$
declare table_name text;
begin
  foreach table_name in array array['prospects','activities','status_history','organization_settings','memberships'] loop
    if not exists (
